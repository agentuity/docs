...
- **New Feature**: Added support for handling inbound email triggers with new `Email` class and `email()` method ([PR #117](https://github.com/agentuity/sdk-js/pull/117))
  - New `email()` method on the Data interface for parsing email content when content type is `message/rfc822`
  - New `Email` class with methods to access email properties: `date()`, `messageId()`, `headers()`, `to()`, `fromEmail()`, `fromName()`, `subject()`, `text()`, `html()`, and `attachments()`
  - Added `mailparser` library dependency for email parsing functionality
