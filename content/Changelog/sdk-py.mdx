...
- **Email Support**: Added comprehensive email parsing capabilities with new `Email` class and `email()` method on request data ([#48](https://github.com/agentuity/sdk-py/pull/48))
- Email class provides access to subject, sender, recipient, date, message ID, headers, text content, HTML content, and attachments
- Dictionary-style access and JSON serialization support for email data
- Content type validation ensures proper email format (`message/rfc822`)
