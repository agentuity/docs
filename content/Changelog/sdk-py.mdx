...
**Changes:**
- **Changed:** Updated documentation to properly reflect v0.0.106 async transition requirements

### ⚠️ Breaking Changes

The Python SDK now requires async/await patterns for accessing request data. This change improves performance and enables better streaming support.
...
+ text_content = await result.data.text()
```

All data accessor methods (`.text()`, `.json()`, `.binary()`, `.base64()`) now return promises and must be awaited.
