...
> ✅ Tip: Set up sandboxes or scenarios to simulate tasks, iterate on prompt engineering, and observe agent decisions.
...
In traditional software engineering, you would build a function that takes a user's input and returns a response.

For example, let's say you have input JSON and your API takes a specific data shape.

```json
{"firstName":"Jeff","lastName":"Bezos"}
```

Traditionally, you would build a function that takes this input, parses it, transforms it and then invokes the API.  There are significant products that have been built to do this.

In an agentic approach, you could just ask the AI to do this for you with a prompt like this:

```ansi
<INSTRUCTIONS>
convert the following JSON to the following to the appropriate 
format for the JSON schema found below:
</INSTRUCTIONS>
...
the keys fromfirstName and lastName to first_name and last_name, respectively.
...
As you can see, the AI has not only converted the JSON to the correct format, but it has also validated that the output matches the schema for you. Agentic software engineering requires you to approach programming with a much different mindset before!
