...
This method creates a named, writable stream that returns immediately, allowing for non-blocking operation and background data processing.
...
export default async function Agent(
  ...
  const stream = ctx.stream.create('llm-response', {
...
export default async function Agent(
  ...
  const stream = ctx.stream.create('progress-updates', {
...
Stream compression reduces bandwidth usage by automatically applying gzip encoding to outbound data:
...
export default async function Agent(
  ...
  const stream = ctx.stream.create('dataset-export', {
    contentType: 'application/json',
...
export default async function Agent(
  ...
  const stream = ctx.stream.create('direct-stream', {
    contentType: 'text/plain'
  });
