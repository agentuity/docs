...
For advanced use cases, you can use the `data` method to send a response with a custom content type.
...
export default async function Agent(
  ...
  ctx: AgentContext
   ...
   const csv = 'name,age\\nAlice,30\\nBob,25';
   return resp.data(csv, 'text/csv');
}`} py={`from agentuity import AgentRequest, AgentResponse, AgentContext

async def run(request: AgentRequest, response: AgentResponse, context: AgentContext):
   csv = 'name,age\\nAlice,30\\nBob,25'
   return response.data(csv, "text/csv")
`} />
