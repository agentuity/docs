...
export default async function Agent(
  ...
  const messageId = await email.sendReply(req, ctx, {
  ...
  ctx.logger.info(\`Reply sent with message ID: \${messageId}\`);
...
The `sendReply` method requires an email authentication token to be present in the request metadata. This token is automatically provided when the agent is triggered by an incoming email.

<CodeExample js={`// JavaScript - The token is automatically extracted from request metadata
const authToken = request.metadata?.['email-auth-token'];
if (!authToken) {
    throw new Error('Email authorization token is required but not found in metadata');
}`} py={`# Python - The token is automatically extracted from request metadata
email_auth_token = request.metadata.get("email-auth-token")
if not email_auth_token:
    raise ValueError("Missing required email-auth-token in AgentRequest metadata for email reply.")
`} />
