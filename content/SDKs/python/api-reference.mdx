...
- `params` (optional): Additional parameters for the object. Only two properties are supported: `content_type` and `content_encoding`. Any other properties are ignored. The `content_type` parameter is optional - if not provided, the content type will be automatically detected based on the data type (e.g., objects will be set to `application/json`). If the content type cannot be determined, it defaults to `application/octet-stream`.
