...
Agent resolution and invocation can throw the following errors:
- **ValueError**: Raised when the specified agent is not found, not accessible, or when agent loop is detected
- **Exception**: Raised for network timeouts, connection failures, HTTP errors, or agent execution failures
- **Timeout Errors**: Specific errors for operations that exceed configured timeout values
- **Connection Errors**: Network-related failures when communicating with remote agents
...
async def handler(request: AgentRequest, response: AgentResponse, context: AgentContext):
    try:
        # Resolve and invoke agent with comprehensive error handling
        agent = resolve_agent(context, {"name": "data-processor"})
        result = await agent.run(request.data)
        return response.json({"success": True, "data": result})
    except ValueError as e:
        # Agent not found or loop detection
        context.logger.error(f"Agent resolution failed: {e}")
        return response.json({"error": "Agent not available", "code": "AGENT_NOT_FOUND"})
    except Exception as e:
        # Network, timeout, or execution errors
        context.logger.error(f"Agent invocation failed: {e}")
        if "timeout" in str(e).lower():
            return response.json({"error": "Request timeout", "code": "TIMEOUT"})
        return response.json({"error": "Service error", "code": "EXECUTION_FAILED"})
```
