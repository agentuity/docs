...
#### `async upsert(name: str, documents: list[dict]) -> list[str]`
...
- `documents`: List of documents to upsert, each with either embeddings or text
...
async def index_products(context: AgentContext, products: List[Dict[str, Any]]) -> List[str]:
    ...
    # Upsert documents to vector storage - pass as single list
    try:
...
# Simple example - Upsert documents with text
ids = await context.vector.upsert("product-descriptions", [
    {"key": "chair_001", "document": "Ergonomic office chair with lumbar support", "metadata": {"category": "furniture"}},
    {"key": "headphones_001", "document": "Wireless noise-cancelling headphones", "metadata": {"category": "electronics"}}
])

# Upsert documents with embeddings
ids2 = await context.vector.upsert("product-embeddings", [
    {"key": "product_123", "embeddings": [0.1, 0.2, 0.3, 0.4], "metadata": {"productId": "123"}},
    {"key": "product_456", "embeddings": [0.5, 0.6, 0.7, 0.8], "metadata": {"productId": "456"}}
])

# Complete example - Index product descriptions with error handling
async def index_products(context: AgentContext, products: List[Dict[str, Any]]) -> List[str]:
    """
    Index product descriptions in vector storage for semantic search.
