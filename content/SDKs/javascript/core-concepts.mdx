...
> **Note:** Agent configuration (name, description, etc.) is managed by the Agentuity CLI and stored in the project configuration file.
...
- `request.contentType` - The content type (or mime type) of the request
...
- `request.object<T>()` - Get the payload as a JSON object and parse it as a specific type
