...
for (const attachment of attachments) {
  console.log('Attachment:', attachment.filename, attachment.contentDisposition);
...
#### `sendReply(request, context, reply): Promise<string>`
...
Returns a `Promise<string>` that resolves to the message ID of the sent reply.
...
async function handler(request, response, context) {
    ...
    const messageId = await email.sendReply(request, context, {
