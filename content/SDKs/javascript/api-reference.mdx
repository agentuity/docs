...
const handler: AgentHandler = async (request, response, context) => {
  ...
  const stream = context.stream.create('llm-response', {
    contentType: 'text/plain',
    metadata: {
      userId,
      sessionId,
...
**Manual Writing Example**

```typescript
const handler: AgentHandler = async (request, response, context) => {
  const { taskId, steps } = await request.data.json();
  
  const stream = context.stream.create('progress-updates', {
  ...
  context.waitUntil(async () => {
    // Stream implements WritableStream, so we can write directly to it
    try {
...
const handler: AgentHandler = async (request, response, context) => {
  ...
  if (action === 'read-direct') {
    ...
    const stream = context.stream.create('data-source', {
      contentType: 'text/plain',
  ...
  if (action === 'stream-reader-response') {
    ...
    const sourceStream = context.stream.create('source-data', {
      contentType: 'application/json',
      metadata: { type: 'generated-data', requestId: request.id }
    });
...
const handler: AgentHandler = async (request, response, context) => {
  ...
  const sourceStream = context.stream.create('input-stream', {
    contentType: 'text/plain',
    metadata: { type: 'input-processing' }
  });
  
  // Create a processing stream for transformed output
  const processedStream = context.stream.create('processed-output', {
    contentType: 'text/plain',
...
const handler: AgentHandler = async (request, response, context) => {
  ...
  const stream = context.stream.create('direct-stream', {
    contentType: 'text/plain',
    metadata: {
