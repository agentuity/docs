...
const handler: AgentHandler = async (request, response, context) => {
  ...
  if (action === 'stream-reader-response') {
    ...
    context.waitUntil(async () => {
      const writer = sourceStream.getWriter();
...
const handler: AgentHandler = async (request, response, context) => {
  ...
  context.waitUntil(async () => {
    ...
    try {
      await sourceWriter.write(sourceData);
    } finally {
      sourceWriter.releaseLock();
      await sourceStream.close();
    }

    // Read from source stream and write processed data to output stream
    const reader = sourceStream.getReader();
    const processedWriter = processedStream.getWriter();
    ...
    try {
      while (true) {
        ...
        await processedWriter.write(processedText);
      }
    } finally {
      reader.releaseLock();
      processedWriter.releaseLock();
