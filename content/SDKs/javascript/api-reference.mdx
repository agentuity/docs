...
- `params` (optional): Additional parameters for the object. Only two properties are supported: `contentType` and `contentEncoding`. Any other properties are ignored. The `contentType` parameter is optional - if not provided, the content type will be automatically detected based on the data type (e.g., objects will be set to `application/json`). If the content type cannot be determined, it defaults to `application/octet-stream`.
