...
- `params` (optional): Additional parameters for the object. The following properties map to canonical HTTP headers: `Content-Type`, `Content-Encoding`, `Cache-Control`, `Content-Disposition`, and `Content-Language`. All other properties are added as generic metadata to the underlying storage provider (if supported). The `Content-Type` parameter is optional - if not provided, the content type will be automatically detected based on the data type (e.g., objects will be set to `application/json`). If the content type cannot be determined, it defaults to `application/octet-stream`.
