...
Let's start with a simple agent that echoes back any data you send it. Notice how it checks `contentType` first to determine the correct parsing method:
...
- ContentType-first parsing (check type before parsing)
- Handling multiple data formats (JSON and text)
- Structured response with timestamp
- Core agent structure (AgentRequest, AgentResponse, AgentContext)

**Key insight:** Real-world agents handle multiple input formats gracefully. Check `contentType` to determine the correct parsing method and handle different data formats.
...
Let's make the agent track total requests using Agentuity's built-in key-value storage:
...
**What changed:**
- Tracks total requests globally
- Uses simple KV key (`total-requests`)
- Uses `context.kv.get()` to read state and `context.kv.set()` to update
- Checks `result.exists` to handle first-time execution
- Returns request count in response statistics
...
**Try it:** Call this agent multiple times with any data. Watch the global request counter increment with each call.
...
Agents need defensive error handling with proper status codes to gracefully handle invalid inputs and system failures:
...
**What changed:**
- Try/catch blocks around data parsing
- Proper HTTP status codes (400 for bad requests)
- Graceful error responses with clear messages
- Defensive storage handling with graceful degradation (continues even if storage fails)
- Clear error logging for debugging

**Key insight:** Agents interact with unpredictable inputs and external systems. Error handling with proper status codes ensures they fail gracefully and provide useful feedback to users.

**Try it:** Test with malformed data to see error handling. Notice how storage failures don't crash the agent - it continues with default values.
...
- Global request tracking with KV storage
...
4. Call the agent multiple times to see the global request counter increment
5. Monitor the logs to see detailed operation tracking
6. Check the key-value storage to see the total request count
...
2. **Test with the clickable prompts** from the `welcome()` function in the examples
3. **Try different inputs:**
   - Valid JSON: `{"user": "Alice", "action": "login"}`
   - Plain text: `"Hello, world!"`
   - Invalid data to test error handling
4. **Call multiple times** to see the global request counter increment
5. **Monitor observability:**
   - Check the Logs tab to see structured logging
   - View the Sessions tab for performance metrics
   - Inspect key-value storage to see the total request count
...
- Sending different JSON objects and text data
- Checking the logs in the DevMode interface
- Calling it multiple times to see the request counter increment
- Testing error cases with malformed data
