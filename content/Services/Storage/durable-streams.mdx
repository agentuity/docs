...
| [Key-Value](/Storage/key-value) | Fast lookups, caching, configuration |
...
Need to create streams from a Next.js backend or other external service? Create authenticated routes that expose stream operations, then call them via HTTP. See [SDK Utilities for External Apps](/Learn/Cookbook/Patterns/server-utilities).
</Callout>
...
- [Key-Value Storage](/Storage/key-value): Fast caching and configuration
