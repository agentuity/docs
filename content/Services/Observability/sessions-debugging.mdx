...
Traditional observability tracks individual HTTP requests. That works for stateless web servers, but agents aren't single requests. A single conversation might span dozens of LLM calls, tool executions, and orchestration steps across multiple interactions.
