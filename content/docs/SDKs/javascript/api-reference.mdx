...
Agent configuration is now managed by the Agentuity CLI and stored in the project configuration file. The `AgentConfig` interface is used internally by the CLI and SDK:
...
> **Note:** You no longer need to export a configuration object from your agent files. Agent configuration is managed through the Agentuity CLI using commands like `agentuity agent create`.
...
The Agentuity SDK provides a server component that can be used to run your agents locally or in a production environment.

### Server Options

The `ServerOptions` interface defines the configuration for the server:
...
interface ServerOptions {
  ...
  autostart?: boolean;
  /**
   * The agents to register with the server
   */
  agents?: {
    /**
     * The unique identifier of the agent
    ...
    id: string;
    /**
     * The path to the agent handler file
     */
    path: string;
  }[];
}
...
You can create a server using the `createServer` function:

```typescript
import { createServer } from '@agentuity/sdk';
  ...
  agents: [
    {
      ...
      path: './src/agents/MyFirstAgent/index.ts'
    },
    {
      id: '7a8b9c0d1e2f3g4h5i6j7k8l9m0n1o2p3q4r5s6t7u8v9w0x1y2z3a4b5c6d7e8f',
      path: './src/agents/MySecondAgent/index.ts'
    }
  ]
});

// Start the server manually if autostart is false
...
The Agentuity SDK includes a router component that handles routing requests to the appropriate agent based on the agent ID. The router is automatically configured based on the agents registered with the server.

When making requests to an agent, you must include the agent ID in the request URL:

```
POST /api/agents/{agentId}
```

This ensures that the request is routed to the correct agent handler.
...
The SDK uses the following environment variables for telemetry configuration:

- `AGENTUITY_OTLP_URL`: The URL of the OpenTelemetry collector
- `AGENTUITY_OTLP_BEARER_TOKEN`: The bearer token for authenticating with the collector
- `AGENTUITY_ENVIRONMENT`: The environment (development, production, etc.)
- `AGENTUITY_SDK_VERSION`: The version of the SDK
- `AGENTUITY_CLI_VERSION`: The version of the CLI
