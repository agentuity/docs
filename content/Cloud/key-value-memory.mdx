...
Key-value memory storage can be integrated with your applications using the Agentuity SDK or 
directly within an agent context. Note that inside agent projects, you will get access to 
this storage with zero setup.  Just use it and go.

### Using Key-Value Memory in Agents

When working within an agent context, you can access key-value storage directly through the context object:
...
export default async function Agent(
  ...
  await ctx.kv.set('my-key', 'my-value');
