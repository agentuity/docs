...
4. **Tag Bot**: Once configured, tag the bot (@AgentuityBot) in Discord messages to forward them to your agent
...
- **Author Information**: Username and ID of the message author
- **Server Context**: Server ID and channel information
- **Timestamp**: When the message was sent
- **Mentions**: Any users or roles mentioned in the message
...
export default async function Agent(
   ...
   ctx: AgentContext
    ...
    if (req.trigger === 'discord') {
        const messageData = await req.data.json();
        ...
        ctx.logger.info('Server: %s, Channel: %s', 
            messageData.guild_id, 
            messageData.channel_id
        );
        
        return resp.text(\`Hello \${messageData.author.username}! I received your message.\`);
    }
...
async def run(request: AgentRequest, response: AgentResponse, context: AgentContext):
    ...
    if request.trigger == 'discord':
        message_data = await request.data.json()
        
        context.logger.info('Discord message from %s: %s', 
            message_data['author']['username'], 
            message_data['content']
        )
        
        # Access server and channel info
        context.logger.info('Server: %s, Channel: %s', 
            message_data['guild_id'], 
            message_data['channel_id']
        )
        
        return response.text(f"Hello {message_data['author']['username']}! I received your message.")
