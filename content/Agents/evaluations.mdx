...
When an agent fails, the problem is often not the model output. It might be a tool call that returned bad data, a state bug that corrupted context, or an orchestration issue that called the wrong agent. Testing just the LLM response misses all of this.

Agentuity evals test the whole run, not just model outputs. And they run on every session in production, not just during development. You catch issues with real traffic, not synthetic test cases.

**The result:**

- **Full-run evaluation**: Test the entire agent execution, not just LLM responses
- **Production monitoring**: Evals run on every session automatically, catching issues with real users
