...
<Callout type="info">
For advanced schema validation patterns including custom validation rules, transformations, error handling, and using alternative libraries like Valibot or ArkType, see the [Schema Validation Guide](/Guides/schema-validation).
</Callout>
...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    c.logger.debug('Detailed processing info', { data: input });

    try {
      const result = await processData(input);

      c.logger.info('Processing successful', {
...
const agent = createAgent({
  ...
  handler: async (c, input) => {
    ...
    // Delete
    await c.kv.delete('user-preferences', userId);
...
**Key Points:**
- `c.kv.get()` returns a result with `exists` flag
- `c.kv.set()` supports optional TTL for expiring data
...
const agent = createAgent({
  ...
  handler: async (c, input) => {
    ...
    if (action === 'index') {
      ...
      const ids = await c.vector.upsert('products', ...documents);
    ...
    const deletedCount = await c.vector.delete('products', ...productIds);
...
const agent = createAgent({
  ...
  handler: async (c, input) => {
    ...
    if (action === 'upload') {
      await c.objectstore.put('documents', filename, data, {
        contentType: 'application/pdf',
        metadata: {
          uploadedBy: c.sessionId,
    ...
    if (action === 'download') {
      const result = await c.objectstore.get('documents', filename);
    ...
    // Delete
    const deleted = await c.objectstore.delete('documents', filename);
...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    const [analyzed, categorized] = await Promise.all([
      c.agent.analyzerAgent.run({ data: enriched }),
      c.agent.categorizerAgent.run({ data: enriched })
    ]);

    // Call final processing agent with combined results
    const final = await c.agent.processorAgent.run({
      analyzed,
      categorized,
      original: input
    });
...
**Key Points:**
- Access agents via `c.agent.agentName.run()`
...
const agent = createAgent({
  ...
  handler: async (c, input) => {
    try {
      c.logger.info(`Processing resource: ${input.resourceId}`);
      ...
      if (error instanceof ValidationError) {
        c.logger.warn(`Validation error: ${error.message}`);
        return {
      ...
      if (error instanceof ResourceNotFoundError) {
        c.logger.warn(`Resource not found: ${error.message}`);
        return {
      ...
      c.logger.error('Unexpected error', error);
      return {
...
async function lookupResource(resourceId: string, ctx: any) {
  const result = await c.kv.get('resources', resourceId);
...
async function processResource(resource: any, ctx: any) {
  c.logger.debug('Processing resource', resource);
...
const agent = createAgent({
  ...
  handler: async (c, input) => {
    ...
    const threadMessages = (c.thread.state.get('messageCount') as number) || 0;
    c.thread.state.set('messageCount', threadMessages + 1);
    ...
    const sessionTotal = (c.session.state.get('totalRequests') as number) || 0;
    c.session.state.set('totalRequests', sessionTotal + 1);
    c.session.state.set('lastActive', Date.now());

    return {
      response: `Processed message ${threadMessages + 1} in this conversation`,
      stats: {
        requestDuration: Date.now() - (c.state.get('startTime') as number),
...
const agent = createAgent({
  ...
  handler: async (c, input) => {
    c.logger.info('Processing task', { task: input.task });
...
agent.addEventListener('completed', (eventName, agent, ctx) => {
  c.logger.info('Agent completed successfully', {
    agentName: agent.metadata.name,
    sessionId: c.sessionId
  });
});
...
const agent = createAgent({
  ...
  handler: async (c, input) => {
    c.logger.info(`Starting workflow: ${input.query}`);
    ...
    const analysis = input.analysisDepth === 'deep'
      ? await c.agent.deepAnalyzer.run({
          data: searchResults,
          includeDetails: true
        })
      : await c.agent.basicAnalyzer.run({
          data: searchResults
        });
    ...
    if (input.includeRecommendations) {
      const recResponse = await c.agent.recommendationEngine.run({
        analysis,
        context: input.query
      });
...
const agent = createAgent({
  ...
  handler: async (c, input) => {
    ...
    if (action === 'stream-with-background') {
      ...
      const stream = await c.stream.create('llm-response', {
        contentType: 'text/plain',
        metadata: {
          userId: userId || c.sessionId,
          model: 'gpt-5-nano',
          type: 'llm-generation'
        }
      });
    ...
    if (action === 'multi-step-stream') {
      const progressStream = await c.stream.create('progress', {
...
router.cron('0 9 * * *', async (c) => {
  c.logger.info('Running daily report generation');
...
router.cron('*/5 * * * *', async (c) => {
  const health = await c.agent.healthCheck.run({});

  if (health.status !== 'ok') {
    c.logger.warn('Health check failed', { status: health.status });
  }
...
router.sse('/updates', (c) => async (stream) => {
  ...
  const updates = await c.agent.processor.run({ task: 'process' });
...
<Callout type="info">
For complete routing documentation including HTTP methods, route parameters, query strings, validation with zValidator, and additional route types like Stream and SMS, see the [Routing & Triggers Guide](/Guides/routing-triggers).
</Callout>
...
<Callout type="info">
For comprehensive evaluation patterns including LLM-as-judge, hallucination detection, RAG quality metrics (contextual relevancy, answer relevancy, faithfulness), and A/B testing, see the [Evaluations Guide](/Guides/evaluations).
</Callout>
...
const agent = createAgent({
  handler: async (c, input) => {
    return c.tracer.startActiveSpan('process-request', async (span) => {
      try {
        // Add attributes to the span
        span.setAttribute('data.type', typeof input);
        span.setAttribute('agent.name', c.agentName || 'unknown');

        // Create a child span for data processing
        return await c.tracer.startActiveSpan('process-data', async (childSpan) => {
          try {
        ...
        c.logger.error('Error processing request', error);
        throw error;
      } finally {
        span.end();
      }
...
- TODO: add a streams demo for v1
