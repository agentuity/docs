...
const agent = createAgent({
  ...
  handler: async (ctx, input) => {
    ...
    if (action === 'upload') {
      await ctx.objectstore.put('documents', filename, data, {
        contentType: 'application/pdf',
        metadata: {
          uploadedBy: ctx.sessionId,
    ...
    if (action === 'download') {
      const result = await ctx.objectstore.get('documents', filename);
    ...
    // Delete
    const deleted = await ctx.objectstore.delete('documents', filename);
...
const agent = createAgent({
  handler: async (ctx, input) => {
    ...
    const [analyzed, categorized] = await Promise.all([
      ctx.agent.analyzerAgent.run({ data: enriched }),
      ctx.agent.categorizerAgent.run({ data: enriched })
    ]);

    // Call final processing agent with combined results
    const final = await ctx.agent.processorAgent.run({
      analyzed,
      categorized,
      original: input
    });
...
**Key Points:**
- Access agents via `ctx.agent.agentName.run()`
...
const agent = createAgent({
  ...
  handler: async (ctx, input) => {
    ...
    const analysis = input.analysisDepth === 'deep'
      ? await ctx.agent.deepAnalyzer.run({
          data: searchResults,
          includeDetails: true
        })
      : await ctx.agent.basicAnalyzer.run({
          data: searchResults
        });
    ...
    if (input.includeRecommendations) {
      const recResponse = await ctx.agent.recommendationEngine.run({
        analysis,
        context: input.query
      });
  ...
  handler: async (ctx, input) => {
    return ctx.tracer.startActiveSpan('process-request', async (span) => {
      try {
        ...
        span.setAttribute('agent.name', ctx.agentName || 'unknown');
...
const agent = createAgent({
  ...
  handler: async (ctx, input) => {
    ...
    const response = await ctx.agent.historyExpert.run({
      question: input.question
    });
