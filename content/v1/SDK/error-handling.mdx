...
const agent = createAgent({
  ...
  handler: async (c, input) => {
    ...
    const user = await getUserById(c.sessionId);
...
const coordinatorAgent = createAgent({
  handler: async (c, input) => {
    ...
    const validation = await c.agent.validatorAgent.run(input);

    // If enrichmentAgent throws an error, it propagates here
    const enriched = await c.agent.enrichmentAgent.run(validation);
...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    try {
      enrichedData = await c.agent.enrichmentAgent.run(input);
    } catch (error) {
...
const agent = createAgent({
  handler: async (c, input) => {
    try {
      const cached = await c.kv.get('cache', input.key);

      if (cached.exists) {
        return { source: 'cache', data: cached.data };
      }
    } catch (error) {
      c.logger.warn('Cache lookup failed, falling back to database', {
        error: error instanceof Error ? error.message : String(error)
      });
    }
    ...
    try {
      await c.kv.set('cache', input.key, data, { ttl: 3600 });
    } catch (error) {
      c.logger.warn('Failed to cache result', { error });
    }
...
const agent = createAgent({
  handler: async (c, input) => {
    try {
      ...
      c.logger.error('Vector search failed', {
        error: error instanceof Error ? error.message : String(error)
      });
...
const agent = createAgent({
  handler: async (c, input) => {
    try {
      await c.objectstore.put('uploads', input.filename, input.data, {
...
const agent = createAgent({
  handler: async (c, input) => {
    c.logger.info('Processing started', { input });

    try {
      const step1 = await performStep1(input);
      c.logger.debug('Step 1 completed', { step1 });

      const step2 = await performStep2(step1);
      c.logger.debug('Step 2 completed', { step2 });
...
const agent = createAgent({
  handler: async (c, input) => {
    const span = c.tracer.startSpan('process-data');
