...
const agent = createAgent('OperationHandler', {
  schema: {
    input: s.object({
...
const coordinator = createAgent('Coordinator', {
  handler: async (ctx, input) => {
    ...
    const validation = await validatorAgent.run(input);

    // If enrichmentAgent throws an error, it propagates here
    const enriched = await enrichmentAgent.run(validation);
...
const agent = createAgent('Processor', {
  handler: async (ctx, input) => {
    ...
    try {
      enrichedData = await enrichmentAgent.run(input);
    } catch (error) {
...
const agent = createAgent('UserLookup', {
  handler: async (ctx, input) => {
...
router.post('/process', processingAgent.validator(), async (c) => {
  try {
...
import actionAgent from '@agent/action';

router.post('/action', actionAgent.validator(), async (c) => {
  try {
    const result = await actionAgent.run(c.req.valid('json'));
    return c.json({ success: true, data: result });
  } catch (error) {
...
```typescript
const agent = createAgent('OutputExample', {
  schema: {
    output: z.object({
...
```typescript
const agent = createAgent('CacheFallback', {
  handler: async (ctx, input) => {
    try {
...
```typescript
const agent = createAgent('SearchFallback', {
  handler: async (ctx, input) => {
    try {
...
const agent = createAgent('FileUploader', {
  handler: async (ctx, input) => {
    try {
      const file = s3.file(`uploads/${input.filename}`);
      await file.write(input.data, { type: input.contentType });

      // Presign URL for download (1 hour)
      const url = s3.presign(`uploads/${input.filename}`, {
        expiresIn: 3600,
        method: 'GET',
      });
...
const agent = createAgent('ProcessingAgent', {
...
This enables centralized error logging, alerting, and tracking across all agents. For comprehensive event handling patterns, see the [Events](/v1/Guides/events) guide.
...
```typescript
const agent = createAgent('StepProcessor', {
  handler: async (ctx, input) => {
...
- Fallback behavior exists (cache → database)
...
test('propagates errors from called agents', async () => {
  // Mock called agent to throw error
  mockAgent.run.mockRejectedValue(new Error('Called agent failed'));

  await expect(
    coordinatorAgent.run(input)
  ).rejects.toThrow('Called agent failed');
});
...
- [Schema Validation](/v1/Guides/schema-validation): Define schemas to validate input and output automatically
- [Events](/v1/Guides/events): Monitor agent lifecycle and handle errors with event listeners
- [Routing & Triggers](/v1/Guides/routing-triggers): Create routes that handle HTTP requests and responses
- [API Reference](/v1/SDK/api-reference): Complete API documentation for context, storage, and utilities
