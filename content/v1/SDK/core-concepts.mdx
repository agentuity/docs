...
handler: async (ctx, input) => {
  ...
  const result = await ctx.agent.enrichmentAgent.run({
    text: input.data
  });

  // Reference to current agent
  const self = ctx.current;
...
handler: async (ctx, input) => {
  ...
  // Object storage
  await ctx.objectstore.put('uploads', 'file.pdf', fileData, {
...
Use `ctx.agent.agentName.run()` to invoke another agent:
...
```typescript
handler: async (ctx, input) => {
  const step1 = await ctx.agent.validator.run(input);
  const step2 = await ctx.agent.enricher.run(step1);
  const step3 = await ctx.agent.analyzer.run(step2);
...
handler: async (ctx, input) => {
  const [webResults, dbResults, cacheResults] = await Promise.all([
    ctx.agent.webSearch.run(input),
    ctx.agent.database.run(input),
    ctx.agent.cache.run(input)
  ]);
...
const parentResult = await ctx.parent.run(input);
```
