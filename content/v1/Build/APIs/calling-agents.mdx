...
// Chat endpoint - uses chatAgent's schema
...
// Summarization endpoint - uses summarizerAgent's schema
router.post('/summarize', summarizerAgent.validator(), async (c) => {
...
// Multi-agent analysis - uses custom schema (different from any single agent)
router.post('/analyze',
  sentimentAgent.validator({
    input: s.object({ content: s.string().min(1) }),
  }),
  async (c) => {
...
Import the agent you're calling and use `agent.validator()` for its schema, or pass a custom schema with `agent.validator({ input: customSchema })` when the API input differs from the agent's schema.
</Callout>
