...
Middleware works on all routes, whether in agent directories (`src/agents/*/route.ts`) or API directories (`src/apis/*/route.ts`). The patterns below apply to both.
</Callout>
...
router.post('/items',
  ...
  async (c) => {
    ...
    await c.kv.set('items', crypto.randomUUID(), data);
...
router.get('/items',
  ...
  async (c) => {
    ...
    return c.json({ page, limit, apiVersion: headers['x-api-version'] });
  }
);
```
...
const rateLimiter = createMiddleware(async (c, next) => {
  ...
  const result = await c.kv.get<number>('ratelimit', key);
  ...
  await c.kv.set('ratelimit', key, count + 1, { ttl: 60 });
  await next();
});
...
- [HTTP Routes](/docs/v1/Build/Routes/http): Complete routing reference
- [Calling Agents from APIs](/docs/v1/Build/APIs/calling-agents): Invoke agents from API routes
