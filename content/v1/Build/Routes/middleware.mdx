...
The `@agentuity/auth` package provides integrated authentication for identity providers like Clerk:
...
router.get('/protected', createMiddleware(), async (c) => {
...
For complete setup including client-side configuration and the Clerk template, see [Adding Authentication](/v1/Build/Frontend/authentication).
</Callout>
...
CORS can also be configured globally via `createApp()`. See [App Configuration](/v1/Get-Started/app-configuration).
...
Compression can also be configured globally via `createApp({ compression: { ... } })`. See [App Configuration](/v1/Get-Started/app-configuration).
