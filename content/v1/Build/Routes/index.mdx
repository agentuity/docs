...
Routes can also expose storage operations to external backends like Next.js or Express. Create authenticated routes that read/write to `c.var.kv`, `c.var.vector`, or `c.var.stream`, then call them via HTTP. See [SDK Utilities for External Apps](/v1/Learn/Cookbook/Patterns/server-utilities) for the complete pattern.
