...
|------|--------|----------|
| [HTTP](/v1/Build/Routes/http) | `router.get()`, `router.post()`, `stream()` middleware | REST APIs, webhooks, LLM streaming |
| [Middleware](/v1/Build/Routes/middleware) | Hono middleware | Auth, logging, validation |
| [Cron](/v1/Build/Routes/cron) | `cron()` middleware | Scheduled tasks |
| [WebSocket](/v1/Build/Routes/websockets) | `websocket()` middleware | Real-time bidirectional |
| [SSE](/v1/Build/Routes/sse) | `sse()` middleware | Server-sent events |
...
Routes can also expose storage operations to external backends like Next.js or Express. Create authenticated routes that read/write to `c.var.kv`, `c.var.vector`, or `c.var.stream`, then call them via HTTP. See [Accessing Storage from External Backends](/v1/Learn/Cookbook/Patterns/external-storage-access) for the complete pattern.
