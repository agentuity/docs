...
router.websocket('/chat', authMiddleware, (c) => (ws) => {
  ...
  ws.onMessage(async (event) => {
    const response = await c.agent.chat.run({
      userId,
...
Use WebSockets when you need **bidirectional** communication. For **server-to-client only** streaming, consider [Server-Sent Events](/docs/v1/Build/Routes/sse).
...
- [Server-Sent Events](/docs/v1/Build/Routes/sse): One-way streaming from server to client
- [HTTP Routes](/docs/v1/Build/Routes/http): Standard request/response endpoints
- [React Hooks](/docs/v1/Build/Frontend/react-hooks): Connect from React with `useAgentWebsocket`
