...
Use SSE when you only need to push data **from server to client**. Use [WebSockets](/docs/v1/Build/Routes/websockets) when you need **bidirectional** communication.
...
router.sse('/builds/:id/logs', (c) => async (stream) => {
  ...
  const logs = await c.kv.get<string[]>('builds', `${buildId}:logs`);
...
- [WebSockets](/docs/v1/Build/Routes/websockets): Bidirectional real-time communication
- [HTTP Routes](/docs/v1/Build/Routes/http): Standard request/response endpoints
- [React Hooks](/docs/v1/Build/Frontend/react-hooks): Connect from React with `useAgentEventStream`
