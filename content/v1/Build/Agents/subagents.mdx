...
const agent = createAgent({
  ...
  schema: {
    input: v.object({
      action: v.picklist(['list', 'add', 'remove', 'count']),
...
const teamAgent = createAgent({
  schema: {
    input: v.object({
      userId: v.string(),
      teamId: v.string(),
      requiredRole: v.optional(v.picklist(['member', 'admin'])),
    }),
    output: v.object({
...
router.post('/add', agent.validator(), async (c) => {
...
- [HTTP Routes](/Build/Routes/http): Complete routing patterns and middleware
