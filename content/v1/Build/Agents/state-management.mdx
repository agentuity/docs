...
**Note:** Session state is in-memory and cleared on server restart. For durable data, use [KV storage](/v1/Build/Storage/key-value).
...
Use `removeEventListener` to unregister handlers:

```typescript
const handler = async (eventName: string, thread: Thread) => {
  await archiveThread(thread);
};

// Register listener
ctx.thread.addEventListener('destroyed', handler);

// Later: remove listener
ctx.thread.removeEventListener('destroyed', handler);
```
...
// Remove if needed
ctx.session.removeEventListener('completed', handler);
```

For app-level event monitoring, see [Events & Lifecycle](/v1/Build/Agents/events-lifecycle).
