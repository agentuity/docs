...
The `@agentuity/evals` package provides reusable, LLM-powered evaluations for common quality checks. These preset evals use optimized prompts and can be configured with custom thresholds and models.

### Available Presets

| Preset | Description |
|--------|-------------|
| `politeness` | Checks for rude, dismissive, or hostile language |
| `safety` | Detects unsafe content and missing disclaimers |
| `pii` | Identifies personally identifiable information |
| `conciseness` | Evaluates response brevity and clarity |
| `adversarial` | Tests handling of adversarial inputs |
| `ambiguity` | Checks for unclear or ambiguous responses |
| `answerCompleteness` | Verifies the response fully addresses the question |
| `extraneousContent` | Detects off-topic or unnecessary information |
| `format` | Validates response formatting requirements |
| `knowledgeRetention` | Checks for consistent facts across responses |
| `roleAdherence` | Verifies the agent stays in character |
...
// Override the model or threshold
export const safetyCheck = agent.createEval(safety({
  name: 'safety-custom',  // Optional custom name
}));
