...
This creates structured log entries you can filter and search in the Agentuity Console or the CLI.
...
router.post('/webhooks/payments', async (c) => {
  ...
  const result = await c.agent.paymentHandler.run({
    event: eventType,
...
Logs are also visible in the Agentuity Console session timeline. See [CLI Reference](/Reference/CLI) for more log viewing options.
...
- [Tracing](/Build/Observability/tracing): Track timing and debug performance with OpenTelemetry spans
- [Sessions & Debugging](/Build/Observability/sessions-debugging): Use session IDs to trace issues across logs and spans