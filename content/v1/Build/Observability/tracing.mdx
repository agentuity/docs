...
router.post('/customers/:id/notify', async (c) => {
  return c.var.tracer.startActiveSpan('send-notification', async (span) => {
    try {
      ...
      const result = await c.agent.notificationSender.run({
        customerId,
        ...body,
      });
...
Traces are also visible in the Agentuity Console, showing the full span hierarchy with timing. See [CLI Reference](/Reference/CLI) for more trace viewing options.
...
- [Logging](/Build/Observability/logging): Learn structured logging patterns
- [Sessions & Debugging](/Build/Observability/sessions-debugging): Use session IDs for debugging