...
const agent = createAgent('FileWriter', {
...
const agent = createAgent('FileReader', {
  handler: async (ctx, input) => {
...
const agent = createAgent('URLGenerator', {
  handler: async (ctx, input) => {
...
const agent = createAgent('LargeFileExporter', {
  handler: async (ctx, input) => {
...
const agent = createAgent('FileHandler', {
  handler: async (ctx, input) => {
...
- [Database](/v1/Build/Storage/database): Relational data with Bun's SQL support
