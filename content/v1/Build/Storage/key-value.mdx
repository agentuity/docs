...
| [Durable Streams](/v1/Build/Storage/durable-streams) | Large exports, audit logs, real-time data |
...
| External backends | HTTP routes | [External Storage Access](/v1/Learn/Cookbook/Patterns/external-storage-access) |
| Frontend | Via routes | [React Hooks](/v1/Build/Frontend/react-hooks) |

<Callout type="info" title="Same API Everywhere">
The KV API is identical in all contexts. `ctx.kv.get()` and `c.var.kv.get()` work the same way. See [Accessing Services](/v1/Reference/sdk-reference#accessing-services) for the full reference.
</Callout>
...
Need to access KV from a Next.js backend or other external service? Create authenticated routes that expose storage operations, then call them via HTTP. See [Accessing Storage from External Backends](/v1/Learn/Cookbook/Patterns/external-storage-access).
</Callout>
...
- [Durable Streams](/v1/Build/Storage/durable-streams): Large data exports
- [State Management](/v1/Build/Agents/state-management): Built-in request/thread/session state
