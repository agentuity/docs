...
For apps with user authentication, wrap `AgentuityProvider` with your identity provider and use `AgentuityClerk` (or similar) to inject auth tokens:
...
Once configured, `useAPI` and `useWebsocket` automatically include auth tokens in requests. See [Adding Authentication](/v1/Build/Frontend/authentication) for full setup.
...
- [React Hooks](/v1/Build/Frontend/react-hooks): Learn about `useAPI`, `useWebsocket`, and `useEventStream`
