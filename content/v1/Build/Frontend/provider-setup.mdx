...
```tsx
// TypeScript knows which agents exist and their types
const { run } = useAgent('chat'); // Autocomplete for agent names
await run({ message: 'Hello' });  // Type-checked input
```
...
- [React Hooks](/v1/Build/Frontend/react-hooks): Learn about `useAgent`, `useAgentWebsocket`, and `useAgentEventStream`
