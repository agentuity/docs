...
For non-React apps (Vue, Svelte, vanilla JS) or server-side code, use the [RPC Client](/v1/Build/Frontend/rpc-client) from `@agentuity/frontend` instead.
</Callout>
...
`useAuth` provides auth-specific state. For non-auth context like `baseUrl`, use `useAgentuity()` instead. See [Authentication](/v1/Build/Frontend/authentication) for integrating with identity providers.
</Callout>
