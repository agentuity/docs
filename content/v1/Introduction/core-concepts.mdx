...
HTTP routes are defined separately in `src/api/index.ts`, which imports agents and calls them via `agent.run(input)`. This separation keeps HTTP routing concerns separate from agent logic.

<Callout type="info">
**Project Structure:** See the [Architecture Guide](/v1/Introduction/architecture) for complete details on file structure, how routes and agents work together, and how the bundler discovers them.
</Callout>
...
v1 supports any [StandardSchema](https://github.com/standard-schema/standard-schema)-compatible library, including Zod, Valibot, and ArkType. Here's an example using the built-in `@agentuity/schema`:
...
Routes are defined in `src/api/index.ts` and import agents directly:
...
router.post('/my-agent', myAgent.validator(), async (c) => {
...
<Callout type="info">
**Advanced Routing:** v1 supports WebSocket, Server-Sent Events, email, cron, and more. See the [Routing & Triggers Guide](/v1/Guides/routing-triggers) for details.
</Callout>
...
Import other agents and call them directly:

```typescript
import { createAgent } from '@agentuity/runtime';
import helperAgent from '@agent/helper';
...
handler: async (ctx, input) => {
  ...
  const turnCount = (ctx.state.get('turnCount') as number || 0) + 1;
  ctx.state.set('turnCount', turnCount);
...
See the [Events Guide](/v1/SDKs/javascript/events) for complete documentation.
...
See the [Evaluations Guide](/v1/SDKs/javascript/evaluations) for complete documentation.

## Next Steps

Now that you understand the core concepts, you can:

- Explore the [Routing & Triggers Guide](/v1/Guides/routing-triggers) for WebSocket, SSE, and specialized routes
- Learn about [Schema Validation](/v1/SDKs/javascript/schema-validation) in depth
- Set up [Evaluations](/v1/SDKs/javascript/evaluations) to test your agents
- Use [Events](/v1/SDKs/javascript/events) for monitoring and analytics
- Check the [API Reference](/v1/SDKs/javascript/api-reference) for complete documentation
