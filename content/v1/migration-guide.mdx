...
const app = createApp();
app.router.post('/my-agent', agent.handler);
...
5. Create an `app` and register your agent with the router
6. Export `app.server` instead of the handler
...
const app = createApp();
app.router.post('/typed-agent', agent.handler);
...
app.router.post('/my-agent', async (ctx) => {
  ...
  // Get query params
  const param = ctx.req.query('param');
...
app.router.email('support@example.com', async (email, ctx) => {
  ...
  // TODO: Email reply functionality is not yet available in v1
...
```typescript
app.router.websocket('/chat', (ctx) => (ws) => {
  ws.onOpen((event) => {
...
```typescript
app.router.sse('/updates', (ctx) => async (stream) => {
...
const app = createApp();
app.router.post('/greet', agent.handler);
...
const app = createApp();
app.router.post('/store', agent.handler);
...
const app = createApp();
app.router.post('/workflow', agent.handler);
...
**Cause**: You're exporting the agent directly instead of the app server.

**Solution**: Make sure you export the app server:
```typescript
const app = createApp();
app.router.post('/agent', agent.handler);

export default app.server; // Not 'agent' or 'handler'
```
...
**Cause**: Agents aren't properly registered with the app router.
...
// Register each agent
app.router.post('/agent1', agent1.handler);
app.router.post('/agent2', agent2.handler);
