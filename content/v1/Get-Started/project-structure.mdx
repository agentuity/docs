...
Creating a new agent is as simple as adding a folder to `src/agents/` with `agent.ts` and `route.ts`. No CLI commands, no manual registration — agents are auto-discovered and registered when you deploy.
</Callout>
...
- [HTTP Routes](/Build/Routes-Triggers/http-routes) — Learn about routing options
