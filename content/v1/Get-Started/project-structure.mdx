...
└── src/
    ...
    │       └── index.ts  # Enables import from '@agent/chat'
...
Agent names come from their directory. Each agent directory contains `agent.ts` (logic) and `index.ts` (enables `import agent from '@agent/name'`). Multiple agents can share a directory.
