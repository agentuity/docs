...
Routes invoke agents via imports: `import agent from '@agent/name'; agent.run(input)`. The SDK auto-discovers agents based on your file structure, providing type-safe calls throughout.
...
For a deep dive on agent planning patterns, see the [Agent Engineering Guide](/v1/Guides/agent-engineering).
...
- Type-safe agent calls via import-based pattern
...
await c.var.kv.get(bucket, key)
await c.var.kv.set(bucket, key, value, { ttl })
await c.var.kv.delete(bucket, key)
...
```typescript
// Structured logging
c.var.logger.info('message', { metadata })
c.var.logger.warn('message', { metadata })
c.var.logger.error('message', { metadata })

// Child loggers (add context to all logs)
const userLogger = c.var.logger.child({ userId: '123' })
...
| Log with context | `c.var.logger` |
| Basic caching | `c.var.kv` |
...
> **Key Insight:** The context object is your toolbox. Use `c.req` for HTTP details, `ctx.logger` for observability, `ctx.kv` for basic caching, and import agents for multi-agent workflows. Each capability is designed to work together seamlessly.
...
1. **The Two-File Pattern:**
   ...
   - Routes call agents via imports: `import agent from '@agent/name'; agent.run()`
...
For more details on routing patterns and best practices, see the [Routing Guide](/v1/Guides/routing).
