...
<Callout type="info">
For a comprehensive overview of agents and how they differ from traditional software, see our [What is an Agent?](/v1/Guides/what-is-an-agent) guide.
</Callout>
...
<Callout type="info">
For deeper insights on this shift, read our [Agent Engineering](/v1/Guides/agent-engineering) guide that covers thinking like an agent builder.
</Callout>
...
<Callout type="info">
Learn more about the Agent-Native Cloud paradigm in our [Agent-Native Cloud](/v1/Guides/agent-native-cloud) guide.
</Callout>
...
- **Built-in persistent memory**: [key-value](/v1/Guides/key-value), [vector](/v1/Guides/vector-db), and [object storage](/v1/Guides/object-storage)
- **Native agent-to-agent communication**: Seamless and secure [channels between agents](/v1/Guides/agent-communication)
- **Built-in observability**: Track agent decisions with [tracing](/v1/Guides/agent-tracing) and OpenTelemetry integration
...
<Callout type="info">
For detailed setup instructions, see our [Getting Started Guide](/v1/Introduction/getting-started) and [CLI Installation Guide](/v1/CLI/installation).
</Callout>
...
<Callout type="info">
DevMode is Agentuity's local development environment that provides instant feedback, complete observability, and a web interface for testing agents. Learn more in our [DevMode Guide](/v1/Guides/devmode).
</Callout>
...
export const agent = createAgent('Hello Agent', {
  // Description helps identify your agent
...
router.post('/greet', async (c) => {
  ...
  const result = await helloAgent.run(body);
...
- **`createAgent('Name', {})`** replaces default export function from v0
- **Schemas** define expected input/output types with Zod
- **Handler signature**: `(ctx, input)` instead of v0's `(request, response, context)`
- **Input is pre-validated** by schema (no manual parsing needed)
- **Direct return values** (not `response.json()`)
- **Routes import and call agents** via `import agent from '@agent/name'; agent.run()`

**Try it:**
1. Create a new project: `npx @agentuity/cli create`
2. Add both files to `src/agent/hello-agent/`
...
**What this demonstrates:**
- `ctx.kv.get(bucket, key)` retrieves data
- `counterResult.exists` checks if data was found
- `ctx.kv.set(bucket, key, value)` stores data
