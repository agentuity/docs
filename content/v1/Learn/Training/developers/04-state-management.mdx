...
<Callout type="info">
**State vs Storage:** This module focuses on **managed state scopes** with automatic lifecycle handling. Module 5 covers **storage APIs** (`ctx.kv`, `ctx.vector`, object storage) for explicit control over data persistence, custom TTL, and shared data *across agents*.
</Callout>
...
**Module 5: Storage APIs** - Learn `ctx.kv`, `ctx.vector`, and object storage for explicit storage control, custom TTL, shared data across agents, and persistence beyond in-memory state.

For more details on state management patterns and lifecycle handling, see the [Sessions & Threads Guide](/v1/Guides/sessions-threads).
