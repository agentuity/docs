...
description: Guide to migrating your Agentuity agents from v0 to v1
---
...
v1 adds native email and cron routes. See [Routes documentation](/v1/Build/Routes) for WebSocket and SSE.
...
Test agent outputs automatically with `agent.createEval()`. See [Evaluations](/v1/Build/Agents/evaluations).

### Event Listeners

Monitor agent lifecycle with `agent.addEventListener('started' | 'completed' | 'errored', ...)`. See [Events](/v1/Build/Agents/events-lifecycle).

### Sessions and Threads

Manage conversational state with `ctx.session` and `ctx.thread`. See [State Management](/v1/Build/Agents/state-management).
...
1. **Check the Documentation**: Visit the [API Reference](/v1/Reference/api-reference) for detailed information
2. **Review Examples**: Browse the [Examples](/v1/Learn/Examples) section for working code
