...
- **Advanced Routing**: Native support for WebSocket, SSE, email, cron, and SMS in `src/api/`
...
- **Database Support**: SQL queries via Bun's native `sql` tagged template literals
- **Native S3 Storage**: Fast file storage via Bun's native S3 APIs (replaces `ctx.objectstore`)
...
// Add an evaluation - name is the first argument
agent.createEval('answer-quality', {
  ...
  handler: async (ctx, input, output) => {
    ...
    return {
      success: true,
      score: isRelevant && isComplete ? 0.95 : 0.5,
      metadata: {
        relevant: isRelevant,
        complete: isComplete
      }
...
agent.addEventListener('completed', async (event, agent, ctx) => {
  ctx.logger.info('Agent completed', { sessionId: ctx.sessionId });
});

agent.addEventListener('errored', async (event, agent, ctx, error) => {
  ctx.logger.error('Agent errored', { error: error.message });
});
...
app.addEventListener('agent.started', async (agent, ctx) => {
  ...
  await analytics.track('agent_started', { session: ctx.sessionId });
});

app.addEventListener('agent.completed', async (agent, ctx) => {
  // Track all agent completions
  await analytics.track('agent_completed', { session: ctx.sessionId });
});
...
// src/api/index.ts
import { createRouter } from '@agentuity/runtime';
import greeter from '@agent/greeter';

const router = createRouter();

router.post('/greet', greeter.validator(), async (c) => {
  const data = c.req.valid('json');
  const result = await greeter.run(data);
  return c.json(result);
});
...
**v1 (agent file):**
```typescript
// src/agent/storage-agent/agent.ts
import { createAgent } from '@agentuity/runtime';
import { z } from 'zod';

const agent = createAgent('Storage Agent', {
  schema: {
    input: z.object({
      userId: z.string(),
...
// src/api/index.ts
import { createRouter } from '@agentuity/runtime';
import storageAgent from '@agent/storage-agent';

const router = createRouter();

router.post('/store', storageAgent.validator(), async (c) => {
  const data = c.req.valid('json');
  const result = await storageAgent.run(data);
  return c.json(result);
});
...
const agent = createAgent('Workflow', {
  handler: async (ctx, input) => {
    // Import and call agents directly
    const processed = await processor.run(input);

    // Call second agent
    const validated = await validator.run(processed);
...
1. **Check the Documentation**: Visit the [v1 API Reference](/v1/SDKs/javascript/api-reference) for detailed information
2. **Review Examples**: Browse the [Examples](/v1/Examples) section for working code
...
1. **Add Schema Validation**: Improve type safety with [Schema Validation](/v1/Build/Routes/http)
2. **Implement Evaluations**: Ensure quality with [Evaluations](/v1/Build/Agents/evaluations)
3. **Use Advanced Routing**: Explore [WebSocket](/v1/Build/Routes/websockets), [SSE](/v1/Build/Routes/sse), and [other routes](/v1/Build/Routes)
4. **Add Event Listeners**: Monitor your agents with [Events & Lifecycle](/v1/Build/Agents/events-lifecycle)
