...
**In agents:** Use `ctx.kv` for programmatic access. See [Key-Value Storage](/v1/Build/Storage/key-value).
...
**In agents:** Use Bun's `s3` API (`import { s3 } from "bun"`). See [Object Storage (S3)](/v1/Build/Storage/object).
...
**In agents:** Use `ctx.vector` for programmatic access. See [Vector Storage](/v1/Build/Storage/vector).
...
**In agents:** Use Bun's `sql` API (`import { sql } from "bun"`). See [Database](/v1/Build/Storage/database).
...
**In agents:** Use `ctx.stream` for programmatic access. See [Durable Streams](/v1/Build/Storage/durable-streams).
