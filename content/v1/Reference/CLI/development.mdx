...
See [Testing with Workbench](/v1/Build/Agents/workbench) for setup and configuration.
...
Agentuity uses a hybrid Bun + Vite architecture during development:

```
┌─────────────────────────────────────────────────────────┐
│  Browser                                                │
│  └── All requests go to http://localhost:3500           │
└─────────────────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────┐
│  Bun Server (port 3500)                                 │
│  ├── Agents, routes, WebSocket                          │
│  ├── Workbench UI                                       │
│  └── Proxies frontend assets → Vite                     │
└─────────────────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────┐
│  Vite Asset Server (dynamic port)                       │
│  ├── Frontend asset transformation                      │
...
- **Single entry point**: All browser traffic goes through port 3500
- **Fast frontend updates**: Vite handles HMR without full server restarts
- **WebSocket support**: Bun's native WebSocket works seamlessly
- **Unified development**: No port switching between API and frontend
