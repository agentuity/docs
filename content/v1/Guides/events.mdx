...
- Use `c.state` to share data between event listeners
...
agent.addEventListener('completed', (eventName, agent, ctx) => {
  ...
  const output = c.state.get('_evalOutput') as { answer: string; confidence: number };

  // Check confidence threshold
  if (output.confidence < 0.7) {
    c.logger.warn('Low confidence output detected', {
  ...
  if (output.answer.length < 10) {
    c.logger.warn('Suspiciously short answer', {
...
**Key Points:**
- Validated input/output stored in `c.state` as `_evalInput` and `_evalOutput`
...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    if (!c.thread.state.has('cleanupRegistered')) {
      c.thread.addEventListener('destroyed', (eventName, thread) => {
        c.logger.info('Cleaning up thread resources', {
          threadId: thread.id,
...
const agent = createAgent({
  ...
  handler: async (c, input) => {
    ...
    if (!c.session.state.has('persistenceRegistered')) {
      c.session.addEventListener('completed', async (eventName, session) => {
        ...
        await c.kv.set('session-metrics', session.id, metrics, {
          ttl: 86400 // 24 hours
        });

        c.logger.info('Session metrics saved', {
          sessionId: session.id,
          metrics
        });
      });

      c.session.state.set('persistenceRegistered', true);
      c.session.state.set('startTime', Date.now());
    }

    // Track session activity
    const totalRequests = (c.session.state.get('totalRequests') as number) || 0;
    c.session.state.set('totalRequests', totalRequests + 1);
    c.session.state.set('lastAction', input.action);
...
agent.addEventListener('started', (eventName, agent, ctx) => {
  c.state.set('performanceStart', performance.now());
});

agent.addEventListener('completed', (eventName, agent, ctx) => {
  const startTime = c.state.get('performanceStart') as number;
  const duration = performance.now() - startTime;

  // Log slow executions
  if (duration > 1000) {
    c.logger.warn('Slow agent execution detected', {
      agentName: agent.metadata.name,
      duration,
      threshold: 1000,
      sessionId: c.sessionId
  ...
  c.logger.info('Agent execution time', {
    agentName: agent.metadata.name,
    duration
  });
});
...
app.addEventListener('agent.errored', async (eventName, agent, ctx, error) => {
  ...
  await c.kv.set('error-logs', errorKey, errorLog, {
    ttl: 604800 // 7 days
  });
});
...
app.addEventListener('agent.completed', async (eventName, agent, ctx) => {
  ...
  // Get current count
  const result = await c.kv.get('analytics', metricsKey);
  let count = 0;
  ...
  // Increment and store
  await c.kv.set('analytics', metricsKey, {
    count: count + 1,
    agentName,
    date: today
  }, {
...
app.addEventListener('agent.started', async (eventName, agent, ctx) => {
  ...
  const logKey = `${auditLog.timestamp}-${c.sessionId}-${agent.metadata.name}.json`;

  await c.objectstore.put('audit-logs', logKey, auditLog, {
...
app.addEventListener('agent.completed', async (eventName, agent, ctx) => {
  ...
  const logKey = `${auditLog.timestamp}-${c.sessionId}-${agent.metadata.name}-completed.json`;
...
- **Use `c.waitUntil()`** - For non-blocking background work, use `c.waitUntil()` to defer processing until after the response is sent
...
Use `c.waitUntil()` to perform heavy work without blocking the response:
...
agent.addEventListener('completed', (eventName, agent, ctx) => {
  ...
  c.waitUntil(async () => {
    ...
    c.logger.info('Metrics sent to external service', {
      agentName: agent.metadata.name,
      sessionId: c.sessionId
    });
  });
  ...
  c.logger.info('Agent completed (handler finished)', {
    agentName: agent.metadata.name
  });
});
