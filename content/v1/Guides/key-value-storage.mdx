...
- **Persistent User Data**: Store user profiles, "remember me" tokens, and preferences that persist beyond active session lifetimes (use `c.session.state` for active session data - see [Sessions and Threads](/Guides/sessions-threads))
...
v1 provides built-in state management (`c.state`, `ctx.thread.state`, `c.session.state`) for data tied to active requests, conversations, and sessions. Use KV when you need:
...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    await c.kv.set('user-sessions', 'user-123', {
...
const agent = createAgent({
  handler: async (c, input) => {
    // Simple store
    await c.kv.set('cache', 'api-response', responseData);

    // Store an object
    await c.kv.set('user-prefs', input.userId, {
      language: 'en',
      timezone: 'UTC'
    });

    // Store with TTL (expires after 1 hour)
    await c.kv.set('sessions', input.sessionId, userData, {
      ttl: 3600  // seconds
    });

    // Store feature flags (no TTL - persistent config)
    await c.kv.set('feature-flags', 'beta-features', {
      darkMode: true,
      aiAssistant: false,
      newDashboard: true
    });
...
const agent = createAgent({
  handler: async (c, input) => {
    // Get a value
    const result = await c.kv.get('user-prefs', input.userId);

    if (result.exists) {
      // Data is already deserialized
      const preferences = result.data;
      c.logger.info('User preferences:', preferences);
    } else {
      c.logger.info('No preferences found for user');
    }

    // Handle missing keys gracefully
    const configResult = await c.kv.get('config', 'app-settings');
...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    const result = await c.kv.get<UserPreferences>('user-prefs', input.userId);
...
const agent = createAgent({
  handler: async (c, input) => {
    // Delete a single key
    await c.kv.delete('sessions', input.sessionId);

    c.logger.info('Session deleted successfully');
...
const agent = createAgent({
  handler: async (c, input) => {
    try {
      const result = await c.kv.get('config', 'settings');

      if (!result.exists) {
        // Initialize with defaults
        await c.kv.set('config', 'settings', defaultSettings);
      }

      return { config: result.exists ? result.data : defaultSettings };
    } catch (error) {
      c.logger.error('Storage error:', error);
...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    await c.kv.set('sessions', input.sessionId, sessionData, {
      ttl: 86400
    });
    ...
    await c.kv.set('rate-limits', input.userId, requestCount, {
      ttl: Math.ceil(secondsUntilHourEnd)
    });

    // Feature flags - no TTL (persistent)
    await c.kv.set('feature-flags', 'global', featureFlags);
