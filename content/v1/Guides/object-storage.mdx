...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    await c.objectstore.put('user-uploads', 'profile-123.jpg', imageData, {
...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    await c.objectstore.put('documents', 'report.pdf', pdfData, {
      contentType: 'application/pdf'
    });

    // Store with full metadata
    await c.objectstore.put('uploads', 'document.pdf', pdfData, {
      contentType: 'application/pdf',
    ...
    await c.objectstore.put('logs', 'agent.log', textData, {
...
const agent = createAgent({
  handler: async (c, input) => {
    // Get an object
    const result = await c.objectstore.get('documents', 'report.pdf');
    ...
    if (result.exists) {
      ...
      c.logger.info(`PDF size: ${pdfData.byteLength} bytes`);
      c.logger.info(`Content type: ${contentType}`);
    } else {
    ...
    if (textResult.exists) {
      ...
      c.logger.info('Log content:', logContent);
    }
...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    c.logger.info(`Download link: ${publicUrl}`);

    // Create with default expiration (1 hour)
    const imageUrl = await c.objectstore.createPublicURL('images', 'photo.jpg');

    // Create short-lived URL (minimum 1 minute)
    const tempUrl = await c.objectstore.createPublicURL(
      'temp-files',
      'preview.png',
...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    if (wasDeleted) {
      c.logger.info('Temporary file cleaned up');
    } else {
      c.logger.info('File was already deleted');
    }
...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    await c.objectstore.put('uploads', key, imageData, {
      contentType: 'image/jpeg'
    });
    ...
    await c.objectstore.put('reports', documentKey, pdfData, {
      contentType: 'application/pdf'
    });
...
const agent = createAgent({
  handler: async (c, input) => {
    ...
    await c.objectstore.put('uploads', filename, input.fileData, {
      contentType
    });
...
const agent = createAgent({
  handler: async (c, input) => {
    try {
      ...
      await c.objectstore.put('uploads', input.key, input.data, {
...
const uploadAgent = createAgent({
  ...
  handler: async (c, input) => {
    ...
    await c.objectstore.put('user-files', key, input.fileData, {
      contentType: input.contentType,
      metadata: {
...
const documentProcessorAgent = createAgent({
  ...
  handler: async (c, input) => {
    ...
    await c.objectstore.put('documents', rawKey, input.rawDocument, {
      contentType: 'application/pdf',
      metadata: {
        'status': 'raw',
    ...
    await c.objectstore.put('documents', processedKey, textData, {
      contentType: 'text/plain',
      metadata: {
        'status': 'processed',
        'document-id': input.documentId,
