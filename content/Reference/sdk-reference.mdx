...
Register cleanup functions that run during graceful shutdown. Useful for packages and libraries that need to clean up resources without modifying app-level shutdown logic.

**Parameters**

- `hook`: Async or sync function to call during shutdown
...
Hooks run after the app's `shutdown` callback and agent shutdowns, in LIFO order (last registered runs first).

**Example: Package-Level Cleanup**

```typescript
// In a package like @agentuity/postgres
...
export function getDatabase() {
  if (!dbPool) {
    dbPool = createPool();
    // Register cleanup without requiring users to wire it up
...
// Register cleanup for a file handle
const file = await Bun.file('data.json').writer();
const unregister = registerShutdownHook(async () => {
  await file.end();
});

// Later, if you close the file early, unregister the hook
await file.end();
unregister();
```
