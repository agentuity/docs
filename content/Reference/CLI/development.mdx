...
After starting with `--inspect`, open `chrome://inspect` in Chrome or use VS Code's debugger to attach. The inspector listens on port 9229 by default.

<Callout type="tip" title="VS Code Debugging">
Create a launch configuration that attaches to the running process:
