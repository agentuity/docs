...
Running `agentuity create` scaffolds a project with:
...
The agent uses the OpenAI SDK routed through Agentuity's AI Gateway, which provides unified billing and observability without requiring separate API keys.

### Project Creation Options

- `--name <name>`: Project name
- `--dir <path>`: Directory to create the project in
- `--template <name>`: Template to use (currently only "default" available)
