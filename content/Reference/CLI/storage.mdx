...
**In agents:** Use `ctx.kv` for programmatic access. See [Key-Value Storage](/Storage/key-value).
...
**In agents:** Use `ctx.vector` for programmatic access. See [Vector Storage](/Storage/vector).
...
**In agents:** Use Bun's `sql` API (`import { sql } from "bun"`). See [Database](/Storage/database).
...
**In agents:** Use `ctx.stream` for programmatic access. See [Durable Streams](/Storage/durable-streams).
...
- [Key-Value Storage](/Storage/key-value): Programmatic KV access in agents
- [Object Storage (S3)](/Storage/object): File storage with Bun S3
- [Database](/Storage/database): SQL queries with Bun SQL
- [Vector Storage](/Storage/vector): Semantic search in agents
