...
export function SessionSidebarSkeleton() {
  ...
  return (
    ...
    >
      ...
      {/* Action */}
      <div className={`p-2 ${isCollapsed ? '' : 'border-b border-white/8'}`}>
        <button
          className={`
            w-full flex items-center gap-2 p-2 rounded-lg
            bg-white/5 text-gray-300 cursor-not-allowed border border-white/10
            justify-start
          `}
          aria-label="Start new chat"
          disabled
        >
          <PenSquare className="w-4 h-4 flex-shrink-0 opacity-60" />
          <span className={`
            text-sm font-medium opacity-60 whitespace-nowrap
            transition-opacity duration-200
            ${isCollapsed ? 'opacity-0 w-0 overflow-hidden' : 'opacity-60'}
          `}>
            New Chat
          </span>
        </button>
      ...
      {/* List */}
      <div className={`
        flex-1 overflow-y-auto p-2 space-y-1 transition-opacity duration-200
        ${isCollapsed ? 'opacity-0 h-0 overflow-hidden pointer-events-none' : ''}
      `}>
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            key={i}
            className="w-full flex items-center gap-2 p-2 rounded-lg justify-start"
          >
